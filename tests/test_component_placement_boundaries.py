...
def test_cannot_place_component_on_vout_row():
    ...
    can_place = b.can_place_component('resistor', b.VOUT_ROW)

    assert not can_place, "Resistor should not be placeable on VOUT_ROW"

    # Try with other component types
    can_place_cap = b.can_place_component('capacitor', b.VOUT_ROW)
    assert not can_place_cap, "Capacitor should not be placeable on VOUT_ROW"

    can_place_nmos = b.can_place_component('nmos3', b.VOUT_ROW)
    ...
    can_place_spanning = b.can_place_component('resistor', b.VOUT_ROW - 1)
...
def test_cannot_place_multipin_spanning_vin_row():
    ...
    can_place_from_vss = b.can_place_component('resistor', b.VSS_ROW)
    ...
    can_place_nmos_from_vss = b.can_place_component('nmos3', b.VSS_ROW)
...
def test_can_place_adjacent_to_vin():
    ...
    can_place_without_wire = b.can_place_component('resistor', b.VIN_ROW + 1)
    ...
    b = b.apply_action(('wire', b.VIN_ROW, b.VIN_ROW + 1))

    # Now placement should work
    can_place_with_wire = b.can_place_component('resistor', b.VIN_ROW + 1)
...
def test_can_place_adjacent_to_vout():
    ...
    can_place_without_wire = b.can_place_component('resistor', start_row)
    ...
    b = b.apply_action(('wire', b.VIN_ROW, start_row))

    # Now placement should work
    can_place_with_wire = b.can_place_component('resistor', start_row)
    assert can_place_with_wire, "Component should be placeable after wire activation"
...
def test_work_area_boundaries_enforced():
    ...
    can_place_without_wire = b.can_place_component('resistor', b.WORK_START_ROW)
    ...
    b = b.apply_action(('wire', b.VIN_ROW, b.WORK_START_ROW))
    can_place_start = b.can_place_component('resistor', b.WORK_START_ROW)
    ...
    b = b.apply_action(('resistor', b.WORK_START_ROW))

    # Now wire to middle row to activate it
    mid_row = b.WORK_START_ROW + 3
    b = b.apply_action(('wire', b.WORK_START_ROW, mid_row))

    # Now we should be able to place on the activated middle row
    can_place_middle = b.can_place_component('capacitor', mid_row)
    ...
    b = b.apply_action(('wire', mid_row, b.WORK_END_ROW - 1))
    can_place_end = b.can_place_component('inductor', b.WORK_END_ROW - 1)
    ...
    can_place_beyond_end = b.can_place_component('resistor', b.WORK_END_ROW)
    ...
    can_place_before_start = b.can_place_component('resistor', b.WORK_START_ROW - 1)
    ...
    can_place_vss = b.can_place_component('resistor', b.VSS_ROW)
    assert not can_place_vss, "Should NOT be able to place component on VSS_ROW"

    can_place_vdd = b.can_place_component('resistor', b.VDD_ROW - 1)
